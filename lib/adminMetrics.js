...
export async function getUserAnalyticsData(userId) {
  ...
  const winningTrades = allUserTrades.filter(trade => trade.result === 'WIN').length;
  const losingTrades = allUserTrades.filter(trade => trade.result === 'LOSS').length;
  ...
  // Calculate daily profits (simplified - assuming amount is the profit/loss)
  const dailyProfits = {};
  allUserTrades.forEach(trade => {
    if (trade.result && trade.open_time) {
      const date = new Date(trade.open_time).toDateString();
      const profit = trade.result === 'WIN' ? toNumber(trade.amount) : -toNumber(trade.amount);
