...
export default function AdminP2POrdersPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (q) params.set('q', q);
      const res = await fetch(`/api/admin/p2p/orders?${params.toString()}`, { cache: 'no-store' });
  ...
  return (
    <div className="p-6 text-gray-200">
      <h1 className="text-xl font-semibold mb-4">P2P Orders</h1>
      <div className="flex items-center gap-3 mb-4">
        ...
        <input value={q} onChange={(e)=>setQ(e.target.value)} placeholder="Search reference/asset/fiat" className="bg-[#0f1320] border border-[#262b40] rounded px-2 py-1 w-72" />
            ...
            <tbody>
              {data.items.map((o) => (
                <tr key={o.id} className="border-t border-[#262b40]">
                  ...
                  <td className="p-2">{o.reference_code || o.id}</td>
                  <td className="p-2">{o.status}</td>
                  <td className="p-2">{o.side}</td>
                  <td className="p-2">{o.amount_asset} {o.asset_symbol}</td>
                  <td className="p-2">{o.amount_fiat} {o.fiat_currency}</td>
                  <td className="p-2">{o.maker?.email}</td>
                  <td className="p-2">{o.taker?.email}</td>
                </tr>
              ))}
            </tbody>
          </table>
          <div className="flex items-center justify-between mt-4">
            <div>Page {data.page} of {Math.max(1, Math.ceil(data.total / data.pageSize))} • {data.total} orders</div>
