      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <Card title="User Analytics">
          <div className="p-6 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-[#101527] p-4 rounded-lg">
                <div className="text-2xl font-bold text-blue-400">{analytics?.users?.new?.toLocaleString?.() || '0'}</div>
                <div className="text-sm text-gray-400">New Users</div>
                <div className="text-xs text-green-400 mt-1">+12.5% vs last period</div>
              </div>
              <div className="bg-[#101527] p-4 rounded-lg">
                <div className="text-2xl font-bold text-green-400">{analytics?.users?.active?.toLocaleString?.() || '0'}</div>
            ...
            <div className="bg-[#101527] p-4 rounded-lg">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-sm text-white">{(() => {
                  const v = analytics?.users?.verified || 0;
                  const t = analytics?.users?.total || 0;
                  const rate = t ? (v / t) * 100 : 0;
              ...
              <div className="w-full bg-[#262b40] rounded-full h-2">
                <div 
                  className="bg-blue-600 h-2 rounded-full" 
                  style={{ width: `${(() => { const v = analytics?.users?.verified || 0; const t = analytics?.users?.total || 0; return t ? (v / t) * 100 : 0; })()}%` }}
                ></div>
              </div>
            </div>
          </div>
        ...
        <Card title="Trading Performance">
          <div className="p-6 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-[#101527] p-4 rounded-lg">
                <div className="text-2xl font-bold text-purple-400">{analytics?.trading?.totalTrades?.toLocaleString?.() || '0'}</div>
                <div className="text-sm text-gray-400">Total Trades</div>
                <div className="text-xs text-green-400 mt-1">+18.2% vs last period</div>
              </div>
              <div className="bg-[#101527] p-4 rounded-lg">
                <div className="text-2xl font-bold text-orange-400">${analytics?.trading?.avgTradeSize ?? 0}</div>
                <div className="text-sm text-gray-400">Avg Trade Size</div>
            ...
            <div className="bg-[#101527] p-4 rounded-lg">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-sm text-white">{analytics?.trading?.successRate ?? 0}%</span>
              </div>
              <div className="w-full bg-[#262b40] rounded-full h-2">
                <div 
                  className="bg-green-600 h-2 rounded-full" 
                  style={{ width: `${analytics?.trading?.successRate ?? 0}%` }}
                ></div>
              </div>
            </div>
          </div>
        ...
        <Card title="System Health & Performance">
          <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="bg-[#101527] p-4 rounded-lg">
                <div className="text-lg font-bold text-green-400">{analytics?.performance?.serverUptime ?? 0}%</div>
                <div className="text-sm text-gray-400">Server Uptime</div>
                <div className="text-xs text-green-400 mt-1">Excellent</div>
              </div>
              <div className="bg-[#101527] p-4 rounded-lg">
                <div className="text-lg font-bold text-blue-400">{analytics?.performance?.avgResponseTime ?? 0}ms</div>
                <div className="text-sm text-gray-400">Avg Response Time</div>
                <div className="text-xs text-green-400 mt-1">Good</div>
              </div>
              <div className="bg-[#101527] p-4 rounded-lg">
                <div className="text-lg font-bold text-yellow-400">{analytics?.performance?.errorRate ?? 0}%</div>
                <div className="text-sm text-gray-400">Error Rate</div>
                <div className="text-xs text-green-400 mt-1">Very Low</div>
              </div>
              <div className="bg-[#101527] p-4 rounded-lg">
                <div className="text-lg font-bold text-purple-400">{analytics?.performance?.activeSessions?.toLocaleString?.() || '0'}</div>
