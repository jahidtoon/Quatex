...
export const calcVolume = (candles: Candle[]): IndicatorOutput[] => {
  const data = candles.map(c => ({
    time: c.time,
    value: c.volume,
    color: c.close >= c.open ? '#26a69a' : '#ef5350' // Green for up, red for down
  })).filter(item => item.value !== null && item.value !== undefined && !isNaN(item.value));
  
  return [{ type: 'histogram', color: '#26a69a', data }];
};
