...
export function useIndicators(chartRef: MutableRefObject<IChartApi | null>, seriesRef: MutableRefObject<ISeriesApi<any> | null>) {
  ...
  const upsertSeries = useCallback((key: IndicatorKey, outputs: IndicatorOutput[], customColors?: string[]) => {
    ...
    const colors = customColors || colorsMap.current.get(key) || outputs.map(o => o.color);
    colorsMap.current.set(key, colors);
    
    let entry = seriesMap.current.get(key);
    if (!entry) {
      try {
        const series = outputs.map((o, i) => {
          const color = colors[i] || o.color;
          if (o.type === 'line') {
            // Special styling for ZigZag
            if (key === 'zigzag') {
              return chart.addLineSeries({ color, lineWidth: 3, lineStyle: 0 } as any);
            }
            return chart.addLineSeries({ color, lineWidth: 2 } as any);
          }
          if (o.type === 'area') return chart.addAreaSeries({ lineColor: color, topColor: 'rgba(0,0,0,0)', bottomColor: 'rgba(0,0,0,0)' } as any);
          if (o.type === 'histogram') return chart.addHistogramSeries({ color, priceScaleId: 'volume' } as any);
      ...
      entry.series.forEach((s, i) => {
        const color = colors[i] || outputs[i]?.color || '#fff';
        try {
          (s as any).applyOptions({ color });
        } catch (error) {
          console.error(`Error updating color for ${key} series ${i}:`, error);
        }
      });
    }
    ...
    entry.series.forEach((s, i) => { 
      if (outputs[i] && outputs[i].data.length > 0) {
        try {
          if (outputs[i].type === 'histogram') {
            // For histogram, include color information
  ...
  const add = useCallback((key: IndicatorKey, candles: Candle[], intervalSec: number, customColors?: string[]) => {
    ...
    try {
      ...
      if (outputs.length) upsertSeries(key, outputs, customColors);
  ...
  const updateColors = useCallback((key: IndicatorKey, colors: string[]) => {
    if (!active.includes(key)) return;
